...
1. Find sample by lab code or sample code/name (50-day window)
...
def main() -> None:
    ...
    render_hero(
        ...
        subtitle=(
            f"Results are stored for <b>{SAMPLE_RETENTION_DAYS} days</b>. Find by lab code or sample, "
        ...
        badge=f"{SAMPLE_RETENTION_DAYS}-day find &amp; edit",
    )
    ...
    render_section_title(
        "1. Find sample",
        f"Search within the {SAMPLE_RETENTION_DAYS}-day retention window, then open to edit results."
    ...
    if do_search:
        if not search_query.strip():
            ...
            if not search_hits:
                st.warning(
                    f"No matching samples in the {SAMPLE_RETENTION_DAYS}-day window."
    ...
    if selected is None:
        if scoped:
            st.info(f"No samples assigned to you in the {SAMPLE_RETENTION_DAYS}-day window.")
        return
    ...
    render_section_title(
        ...
        (
            ...
            if is_micro
            else f"Pick a test, edit inputs if needed, then Calculate & save (overwrites within {SAMPLE_RETENTION_DAYS} days)."
        ),
    )
    ...
    if is_micro:
        ...
        formula_text = nutrition_formula_display(choice, prior_inputs) if choice.startswith("bn_") else (test.formula_display or "")
        formula_lines = [p.strip() for p in formula_text.split(";") if p.strip()]
        ...
        with st.form("worksheet_form"):
            ...
            protein_titrant = (
                str(prior_inputs.get("titrant") or PROTEIN_TITRANT_NAOH)
            ...
            for field in test.inputs:
                ...
                if field.field_type == "choice":
                    ...
                    if choice == "bn_protein" and field.key == "titrant":
                        protein_titrant = str(inputs[field.key])
    ...
    render_section_title(
        ...
        f"Saved results from DB — reopen this sample anytime within {SAMPLE_RETENTION_DAYS} days to edit.",
    )
    if not results:
    ...
    if st.button("Generate protocol document", type="primary"):
        if missing_saves:
            st.error(
                "Cannot generate protocol until every selected test is calculated and saved: "
                + ", ".join(missing_saves)
            )
        elif header is None or not (header.protocol_no or "").strip():
            st.warning(
