...
def format_report_footer_metadata(
    generated_by: str = "",
    generated_at: str = "",
) -> str:
    """Left footer line: Generated by + date/time for final test reports."""
    parts: list[str] = []
    if (generated_by or "").strip():
        parts.append(f"Generated by: {generated_by.strip()}")
    if (generated_at or "").strip():
        parts.append(f"Date/time: {generated_at.strip()}")
    return "  |  ".join(parts)
