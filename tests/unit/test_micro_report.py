...
def test_micro_result_calculator():
    ...
    display, numeric = test.calculate({"result_obs": "Absent"}, {})
...
def test_fill_micro_report_docx():
    ...
    # Results table: header + section + 6 tests
    assert len(doc.tables) >= 2
    results_table = doc.tables[1]
    assert len(results_table.rows) == 8
    cell_texts = [
        results_table.rows[i].cells[1].text.strip() for i in range(2, 8)
    ]
    ...
    assert "3.0 x 10³ cfu/gm" in results_table.rows[2].cells[2].text
